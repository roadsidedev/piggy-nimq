...
// Build file as array of lines to avoid any escaping issues
const content = [
  ...
  '    const _asset = await MockERC20_.deploy("USD Coin", "USDC", 6);',
  '    await _asset.waitForDeployment();',
  '    const assetAddr = await _asset.getAddress();',
  '',
  '    const MockAA_ = await ethers.getContractFactory("MockAaveAdapter");',
  '    const _mockAdapter = await MockAA_.deploy(assetAddr);',
  ...
  '      await vault.connect(user).deposit(amount, user.address);',
  '      await vault.connect(user).enableYield(amount);',
  '      await expect(() => vault.connect(borrower).borrow(ethers.parseUnits("5000", 6), borrower.address)',
  '      ).to.changeTokenBalance(asset, borrower, ethers.parseUnits("5000", 6));',
  '    });',
  ...
  '      await vault.connect(user).deposit(amount, user.address);',
  '      await vault.connect(user).enableYield(amount);',
  '      await vault.connect(borrower).borrow(ethers.parseUnits("5000", 6), borrower.address);',
  '      expect(await vault.debtShares(borrower.address)).to.equal(ethers.parseUnits("5000", 6));',
  '      expect(await vault.totalDebt()).to.equal(ethers.parseUnits("5000", 6));',
  '    });',
  ...
  '      await vault.connect(user).enableYield(amount);',
  '      await vault.connect(borrower).borrow(ethers.parseUnits("5000", 6), borrower.address);',
  '      await asset.connect(borrower).approve(await vault.getAddress(), ethers.parseUnits("2000", 6));',
  '      await vault.connect(borrower).repay(ethers.parseUnits("2000", 6), borrower.address);',
  '    });',
